...
from cwsetup import chipwhisperersetup, reset_target, randbytes, read_sig, log_info, elapsed_simpleserial, read_root
...
def run_exp1(target, scope, inplength, N, M, ext_idx = 0, logged=False):
    """
    ...
    try:
        ...
        for idx in range(N):
            ...
            for i in range(M):
                ...
                inp = b"\x00"*zeropad + randbytes(inplength, seed=idx+ext_idx)
                ...
                if ret: # In case of time out
                    ...
                    # 6. Read signature
                    sig = read_sig(target, 35+3)
                    root = read_root(target)
...
def run_exp_expl(logged=False):
    ...
    exp_out = "05b7a06900e52fd2c6778a312fb95fdf"
    TIMES = 10
...
try:
    ...
    run_exp1(target, scope, inplength=3, N=5, M=1024, ext_idx = 9, logged=LOG_BY_DEFAULT)


    # DURATION = elapsed_simpleserial(target, 'x', b'\x00'*8)
